...
[source,bash]
----
./macos-pyinstaller.sh -m -b -w
----

[source,text]
----
Running pyi-makespec --osx-bundle-identifier edu.buffalo.hwi.polo --icon ../macOS/application.icns --windowed --specpath macOS --collect-all tensorflow --collect-all pptx --add-data ../src/data:data --add-data ../src/astor:astor --add-data ../src/unrar:unrar --add-data ../src/templates:templates src/Polo.py
...
[source,bash]
----
platform=x64; pyinstaller --clean -y macOS/Polo.spec 2>&1 | tee pyinstall-"${platform}"-output.txt
----

>**NOTE:** Use platform=x64|arm64
